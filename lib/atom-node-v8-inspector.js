...
export default {
    ...
    activate(state)
    {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'atom-node-v8-inspector:clear-entry-point-(use-active-file)': () => this.clearEntryPoint()
        }));

        this.subscriptions.add(atom.commands.add('atom-text-editor', {
            'atom-node-v8-inspector:toggle-breakpoint': () => this.toggleBreakpoint(),
            'atom-node-v8-inspector:set-current-file-as-entry-point': () => this.setCurrentFileAsEntryPoint()
        }));
